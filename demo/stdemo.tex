...
One could define a single function to initialize all fields (using a key=val interface), but, thinking of a more traditional way (and hiding the implementation from end user) one could set two functions to start the initialization process \tsobj{\NewActivity,\ActivitySet}. 
\tscode*[stdemo]{ActivityFunctions}[1]

The idea being to use one right after the other, though, once created \tsobj{\NewActivity} it can be, at a later point in time ``initialized'' using the optional argument from \tsobj{\ActivitySet}
