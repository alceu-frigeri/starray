...
This can be used after any command which 'parses a term', for instance \tsobj{\starray_term_parser:n}, see section \ref{conditionals:terms}, but it only makes sense (and returns a reliable/meaningful result) IF the last parser operation was successfully executed.
\end{tsremark}
...
\end{codedescribe}
\begin{tsremark}[\color{red}Warning:]
\tsobj[marg]{parsed-refA,parsed-refB} are the values returned by\break  \tsobj{\starray_term_parser:nNN}.
\end{tsremark}

\begin{codedescribe}[code,new=2025/10/18]{\starray_parsed_get_iter:N,\starray_parsed_get_iter:NTF,\starray_parsed_get_iter:NNN}
\begin{codesyntax}%
...
\end{codesyntax}
These will save the iterator’s value (of a parsed term) in a integer variable (\tsobj[pkg]{expl3}). The \tsobj[marg]{if-true,if-false} regards the status of the last \tsobj{\starray_term_parser:} command.
\end{codedescribe}
\begin{tsremark}[\color{red}Warning:]
Thes can be used after any command which 'parses a term', for instance \tsobj{\starray_term_parser:n}, see section \ref{conditionals:terms}, but it only makes sense (and returns a reliable/meaningful result) IF the last parser operation was successfully executed.
\end{tsremark}
\begin{tsremark}[\color{red}Warning:]
\tsobj[marg]{parsed-refA,parsed-refB} are the values returned by\break  \tsobj{\starray_term_parser:nNN}.
\end{tsremark}
...
\begin{codedescribe}[code,new=2025/10/18]{\starray_parsed_get_cnt:N,\starray_parsed_get_cnt:NTF}
\begin{codesyntax}%
\tsmacro{\starray_get_cnt:N}{integer}
\tsmacro{\starray_get_cnt:NTF}{integer,if-true,if-false}
\end{codesyntax}
Similarly to \tsobj{\starray_get_cnt:nN,\starray_get_cnt:nNTF} these will save  the number of terms (of the last parsed term) in a integer variable (\tsobj[pkg]{expl3}). The \tsobj[marg]{if-true,if-false} regards the status of the last \tsobj{\starray_term_parser:} command.
\end{codedescribe}
\begin{tsremark}
A warning might be raised (see \ref{pack:options}) in case the last \tsobj{\starray_term_parser:} failed. The branching version doesn't raise any warning.
\end{tsremark}
...
\begin{codedescribe}[code,new=2025/10/17]{\starray_parsed_get_prop:nN}
\begin{codesyntax}%
\tsmacro{\starray_parsed_get_prop:nN}{key,tl-var}
\end{codesyntax}
\tsmacro{\starray_parsed_get_prop:nN}{key,tl-val} stores the value of \tsobj[marg]{key}, if it exists, from the last parsed term. This comes handy if one has to retrieve many keys from the same term.
\end{codedescribe}
\begin{tsremark}[\color{red}Warning:]
This can be used after any command which 'parses a term', for instance \tsobj{\starray_term_parser:n}, see section \ref{conditionals:terms}, but it only makes sense (and returns a reliable/meaningful result) IF the last parser operation was successfully executed.
\end{tsremark}
...
\end{codedescribe}
\begin{tsremark}[\color{red}Warning:]
\tsobj[marg]{parsed-refA,parsed-refB} should be the values returned by \break \tsobj{\starray_term_parser:nNN}.
\end{tsremark}
...
\begin{codedescribe}[code,new=2023/05/20,update=2025/10/17]{\starray_term_parser:n,\starray_term_parser:nTF}
\begin{codesyntax}%
...
\begin{codedescribe}[code,new=2023/11/28,update=2025/10/17]{\starray_term_parser:nNN,\starray_term_parser:nNNTF}
\begin{codesyntax}%
...
\end{codesyntax}
Similar to the ones above (\tsobj{\starray_term_parser:n}). \tsobj[marg]{parsed-refA,parsed-refB} (assumed to be two token list vars, \tsobj[meta]{tl-var}) will receive two 'internal references' that can be used in commands like \tsobj{\starray_parsed_...:NN} which expects such 'references'. The assignment is global.
\end{codedescribe}
\begin{tsremark}
...
\end{tsremark}
\begin{tsremark}[\color{red}Warning:]
This can be used after any command which 'parses a term', for instance \tsobj{\starray_term_parser:n}, but it only makes sense (and returns a reliable/meaningful result) IF the last parser operation was successfully executed.
\end{tsremark}
